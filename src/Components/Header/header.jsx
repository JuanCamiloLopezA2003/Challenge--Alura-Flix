...
export default function Header({ onSearch }) {
    ...
    const NoMostrarBuscador = location.pathname.startsWith('/mirarser/') || location.pathname === '/video' || location.pathname === '/genero' || location.pathname === '/404';
    const NoMostrarGenero = location.pathname.startsWith('/mirarser/') || location.pathname === '/' || location.pathname === '/404';

    return (
        <header className='header'>
            <nav className='nav'>
                <Link to="/">
                    <img className='logo' src={logoimg} alt="logo-AluraFlix" />
                </Link>
                        ...
                        <>
                            <li>
                                <Link to="/video">
                                    ...
                                    <span className="text-home">NUEVA SERIE</span>
                                </Link>
                            </li>
                            {!NoMostrarGenero && (
                                <li>
